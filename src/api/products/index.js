...
productsRouter.post("/", async (req, res, next) => {
  ...
  productsRouter.get("/", async (req, res, next) => {
    try {
      ...
      res.send({ links: mongoQuery.links("http://localhost:3005/products", total), total, totalPages: Math.ceil(total / mongoQuery.options.limit), products })
    } catch (error) {
      next(error)
    }
  })
  ...
  productsRouter.get("/:productId", async (req, res, next) => {
    try {
      ...
      if (product) {
        ...
        next(createHttpError(404, `Productwith id ${req.params.productId} not found!`))
      }
    } catch (error) {
      next(error)
    }
  ...
  productsRouter.put("/:productId/reviews/:reviewId", async (req,res,next)=>{
    ...
    else{next(createHttpError(404, `review with Id ${req.params.productId} not found`))
  }
    }else{
        next(createHttpError(404, `product with Id ${req.params.productId} not found`))
      }
